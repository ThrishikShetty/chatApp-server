...
const newGroupChat = TryCatch(async (req, res, next) => {
  ...
  if (members.length < 2)
    return next(new ErrorHandler("Please add more than 2 members", 400));

  // All users + the creater user req.uer
...
const getMyChats = TryCatch(async (req, res, next) => {
  //to get grp chat that   user is present
  // here we have mem id so we have to get their name and avatar by populate function
...
const getChatDetails = TryCatch(async (req, res, next) => {
  if (req.query.populate === "true") {
    const chat = await Chat.findById(req.params.id)
      .populate("members", "name avatar")
      .lean(); // lean to save the populate but not permanenlty in databasre
